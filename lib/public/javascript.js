...
(function () {
  ...
  function render(data) {
    ...
    setTimeout(() => {
        // Create a promise that resolves when MathJax is done, or immediately if MathJax isn't present.
        const mathjaxPromise = window.MathJax ? window.MathJax.typesetPromise() : Promise.resolve();

        mathjaxPromise.catch((err) => {
            console.error("MathJax typesetting error:", err);
        }).finally(() => {
            // This block runs after MathJax is done (or has failed or isn't there).
            // It is now safe to restore focus and scroll positions.
            if (focusedElementId) {
