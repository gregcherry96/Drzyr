...
(function () {
  ...
  function renderNavbar(config) {
    ...
    if (!config || (!config.left && !config.center && !config.right)) {
      navbarContainer.classList.remove('navbar-visible');
      return;
    }
    navbarContainer.classList.add('navbar-visible');

    const createIcon = (iconClass) => createElement('i', { className: `icon ${iconClass} mr-1` });

    const processItem = (item) => {
      let element;
      switch (item.type) {
        case 'navbar_brand':
          element = createElement('a', { href: '#', className: 'navbar-brand mr-2', innerText: item.text });
          if (item.icon) {
            element.prepend(createIcon(item.icon));
          }
          return element;
        case 'navbar_link':
          element = createElement('a', {
            href: item.href,
            className: `btn btn-link ${window.location.pathname === item.href ? 'navbar-link-active' : ''}`,
            innerText: item.text
          });
          if (item.icon) {
            element.prepend(createIcon(item.icon));
          ...
          return element;
        case 'dropdown':
          const dropdownContainer = createElement('div', { className: 'dropdown' });
          const toggle = createElement('a', { href: '#', className: 'btn btn-link dropdown-toggle', tabIndex: 0 });
          toggle.innerText = item.label;
          if (item.icon) {
            toggle.prepend(createIcon(item.icon));
          }
          const dropdownMenu = createElement('ul', { className: 'menu' });
          item.items.forEach(menuItem => {
            if (menuItem.type === 'divider') {
              dropdownMenu.appendChild(createElement('li', { className: 'divider' }));
            } else {
              const li = createElement('li', { className: 'menu-item' });
              li.appendChild(createElement('a', { href: menuItem.href, innerText: menuItem.text }));
              dropdownMenu.appendChild(li);
            }
          });
          dropdownContainer.append(toggle, dropdownMenu);
    ...
    ['left', 'center', 'right'].forEach(sectionName => {
      if (config[sectionName] && config[sectionName].length > 0) {
        const sectionEl = createElement('section', { className: 'navbar-section' });
        config[sectionName].forEach(item => sectionEl.appendChild(processItem(item)));
        navbarContainer.appendChild(sectionEl);
      }
    });
  }
