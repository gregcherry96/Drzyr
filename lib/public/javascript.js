...
(function () {
  ...
  function renderNavbar(config) {
    ...
    // If no config, ensure the visibility class is removed and stop.
    if (!config) {
    ...
    // ** THE FIX IS HERE **
    // Add a class to make the container visible and apply styles.
  ...
  const widgetCreators = {
    ...
    data_table(element) {
      const container = createElement('div', { id: element.id });

      // Defer initialization to ensure the container is in the DOM
      setTimeout(() => {
