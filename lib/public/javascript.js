...
(function () {
  ...
  function renderNavbar(elements) {
    ...
    const leftSection = createElement('section', { className: 'navbar-section' });
    const rightSection = createElement('section', { className: 'navbar-section' });

    elements.forEach(element => {
      let el;
      if (element.type === 'navbar_brand') {
        el = createElement('a', { href: '#', className: 'navbar-brand mr-2', innerText: element.text });
        leftSection.appendChild(el);
      } else if (element.type === 'navbar_link') {
        el = createElement('a', {
          href: element.href,
          className: `btn btn-link ${window.location.pathname === element.href ? 'navbar-link-active' : ''}`,
          innerText: element.text
        });
        // A simple way to float links to the right could be added here if needed
        leftSection.appendChild(el);
      }
    });

    navbarContainer.append(leftSection, rightSection);
  }
