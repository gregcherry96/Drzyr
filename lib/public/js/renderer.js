...
window.DrzyrRenderer = (function(window) {
  ...
  const widgetCreators = {
    // A new widget creator for displaying runtime errors from the server.
    error_display(el) {
        const template = `
            <div class="form-group" style="padding: 1rem; border: 1px solid #e83e8c; background: rgba(232, 62, 140, 0.05);">
                <h4 style="color: #e83e8c;">Runtime Error: {{message}}</h4>
                <pre style="white-space: pre-wrap; font-size: 0.8em; color: #d1d1d1; background: #2b2b2b; padding: 1rem; border-radius: 4px;"><code>{{backtrace}}</code></pre>
            </div>
        `;
        return { widget: fromHTML(Mustache.render(template, el)), skipLabel: true };
    },
  ...
  return {
    render(data) {
        ...
        // Handle top-level errors sent from the server.
        if (data.error) {
            app.innerHTML = '';
            sidebar.innerHTML = '';
            const errorCreator = widgetCreators[data.error.type];
            if (errorCreator) {
                const errorElement = errorCreator(data.error);
                if (errorElement) app.appendChild(errorElement.widget || errorElement);
            }
